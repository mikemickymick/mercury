...
async function PopulateProductBuilder(chatMaster, personalWord) {
  ...
  const chatComposition = GenerateChatComposition(chatObjArr);
  const timeArray = GenerateMessageTimes(chatObjArr);
  const dayArray = GenerateMessageDays(chatObjArr);
  const firstEncounter = GenerateFirstEncounter(chatObjArr);
  const personalWordSearchRecord = GenerateSearchRecord(chatObjArr, 'personal', false, 2, 1, [personalWord]);
  const fromDateStr = firstEncounter.FirstMessageDate;
  const toDateStr = chatObjArr[chatObjArr.length - 1].Date;
